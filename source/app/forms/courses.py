from django.forms import ModelForm
from crispy_forms.helper import FormHelper
from crispy_forms.layout import Field, Layout, Submit, HTML
from crispy_forms.bootstrap import FormActions

from app.models.courses import Course, CourseMaterial, CoursePlan


class CourseForm(ModelForm):
    class Meta:
        model = Course
        fields = ('id', 'name', 'code', 'start_date', 'end_date', 'duration', 'exam_date', 'evaluation_date', 'published', 'cclass', 'teachers')

    def __init__(self, *args, **kwargs):
        super(CourseForm, self).__init__(*args, **kwargs)

        self.fields['name'].required = True
        self.fields['code'].required = True
        self.fields['start_date'].required = True
        self.fields['end_date'].required = True
        self.fields['duration'].required = True
        self.fields['exam_date'].required = True
        self.fields['evaluation_date'].required = True
        self.fields['published'].required = True
        self.fields['cclass'].required = True
        self.fields['teachers'].required = True

        self.helper = FormHelper(self)
        self.helper.form_method = 'POST'
        self.helper.form_class = 'form-horizontal'
        self.helper.label_class = 'col-md-2 small'
        self.helper.field_class = 'col-md-8'
        self.helper.layout = Layout(
            Field('name', css_class='input-sm'),
            Field('code', css_class='input-sm'),
            Field('start_date', css_class='input-sm'),
            Field('end_date', css_class='input-sm'),
            Field('duration', css_class='input-sm'),
            Field('exam_date', css_class='input-sm'),
            Field('evaluation_date', css_class='input-sm'),
            Field('published', css_class='input-sm'),
            Field('cclass', css_class='input-sm'),
            Field('teachers', css_class='input-sm'),
            FormActions(
                Submit('save_changes', 'Save changes', css_class='btn-primary btn-sm'),
                HTML('<input type="button" name="Cancel" value="Cancel" class="btn btn-default btn-sm" onclick="javascript:window.location='+ "'" +'/dashboard/class/'+ "'" +';" />')
            ),
        )



class CourseMaterialForm(ModelForm):
    class Meta:
        model = CourseMaterial


class CoursePlanForm(ModelForm):
    class Meta:
        model = CoursePlan
